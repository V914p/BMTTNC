    ...
    while True:
        try:
            ...
            if not encrypted_message:
                ...
                break  # Thoát khỏi vòng lặp nếu client đã ngắt kết nối

            decrypted_message = decrypt_message(aes_key, encrypted_message)
            print(f"Received from {client_address}: {decrypted_message}")

            # Phát tán dữ liệu cho các client khác
            ...
            print(f"Connection bị ngắt đột ngột từ client {client_address}")
            break  # Thoát khỏi vòng lặp nếu kết nối bị reset

        except socket.timeout:
            print(f"Timeout xảy ra với client {client_address}")
            break  
